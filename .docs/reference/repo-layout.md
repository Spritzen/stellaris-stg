...
tools/
  ...
  fix_prescripted_rooms.py    rewrites `room =` from STNH's own assignment
                        ...
                        built tree. Four of them (gen_home_systems,
                        gen_ruler_clothes, gen_star_names, fix_ship_locators)
                        also read stg-build/ to ask what the merge already
                        declares; a generator that does must exclude its own
                        previous output, or it subtracts itself (decision 81)
