...
| **STNH art** | Take the art paths, never the script. The ship tree is pruned to the directories declarations actually name ([17](../decisions/17-walshicus-shipsets-replace-stnh-hulls.md)); unreferenced files are pruned by the closure ([43](../decisions/43-clutter-pass.md)); everything else is taken whole. [Details](../architecture/stnh-art.md). |
| **Ship art** | **17 of the 22 cultures the majors, quadrant and frontier powers fly** are Walshicus' vanilla-chassis shipsets; five (BAJ, TRI, ADR, BOL, BRE) stay on generated STNH hulls. [Decision 17](../decisions/17-walshicus-shipsets-replace-stnh-hulls.md) moved nine of the then-fourteen and superseded [16](../decisions/16-stnh-shipsets-on-a-vanilla-chassis.md) in part; the eight frontier powers of Phase 2 exist *because* the other Walshicus sets were already in the tree. |
| **Borg & Dominion** | Playable prescripted empires. No crisis scripting. |
| **Trek map** | Trek-named systems in a normally generated galaxy, **and home systems ARE placed**. The no-hand-placed-homeworlds half was wrong in a way only play showed: `system_name` is a *label*, so "Sol" was a label on a random system whose other planets came from the Federation's `planet_names` pool — Bajor and Andoria among them. 37 empires now name a real `initializer`. [Decision 23](../decisions/23-real-home-systems.md). **The "normally generated galaxy" half no longer holds, and this is the record of the change**: six galaxies contained no Trek AI empire, because the mechanism that places them is a **static galaxy scenario** rather than the spawn lottery — [decision 85](../decisions/85-create-country-initializers.md). STG shipped its own map on 2026-08-27 — 95 systems, 21 empires ([86](../decisions/86-static-galaxy-scenario.md)) — with its lanes generated into the file ([87](../decisions/87-static-map-lanes-are-generated.md)), and the picker offers STG's own scenarios and nothing else ([88](../decisions/88-lock-the-galaxy-picker.md)). **Since 2026-09-01 there are three of them** — the same map at 95, 600 and 1,000 systems ([111](../decisions/111-three-galaxy-sizes.md)) — so the player chooses a size and never a shape. **STG's galaxy is a hand-placed map, not a generated one.** [Phase 6](phases.md#phase-6--the-static-galaxy). |
| **Source backup** | **`.source/` — a pinned copy of every source mod, and the build's only input.** `make vendor` never reads `/workshop`. Free: same btrfs subvolume, so it reflinks. [Decision 08](../decisions/08-source-snapshot.md). |
