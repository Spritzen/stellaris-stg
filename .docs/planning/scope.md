...
| **Trek map** | Trek-named systems in a normally generated galaxy, **and home systems ARE placed**. The no-hand-placed-homeworlds half was wrong in a way only play showed: `system_name` is a *label*, so "Sol" was a label on a random system whose other planets came from the Federation's `planet_names` pool — Bajor and Andoria among them. 37 empires now name a real `initializer`. [Decision 25](../decisions/25-real-home-systems.md). |
