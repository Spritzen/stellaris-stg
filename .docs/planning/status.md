...
| | |
|---|---|
| `src/map/setup_scenarios/stg_alpha_beta_quadrant*.txt` | **three sizes since 2026-09-01** ([111](../decisions/111-three-galaxy-sizes.md)) — **95 / 600 / 1,000 systems** over a radius of **218 / 399 / 448**, 21 empires in every one, every coordinate harvested from STNH's default galaxy map and scaled. Small is byte-identical to the map three runs graded and keeps `default = yes`. The lanes are generated from those same positions — shipping without them, as 21 of STNH's 22 maps appear to, cost the 2026-08-27 run ([87](../decisions/87-static-map-lanes-are-generated.md)) |
...
| `check_static_galaxy` | five questions as shipped; **six since [107](../decisions/107-the-ai-federation.md)** — vanilla floor **0**, STNH's own maps **4,265**. Population **3 scenarios** since the sizes landed, and connectivity is re-proved over all **3,034** lanes |
