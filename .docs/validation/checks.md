...
| `check_static_galaxy` | a static galaxy scenario's five joins: every `initializer` and `spawn_design` resolves; every country flag a `spawn_weight` tests is reachable, from `set_country_flag` **or** from `common/prescripted_flags/`; no two systems share an id or a position; hyperlane endpoints are declared and the graph is connected, and a static map declaring **no** lanes is an error; and every `flag = empire_X` on a prescripted empire resolves to an entry carrying that empire's own key | every one of them fails silently, and STNH's 22 maps report **4,265** against their own tree — 4,256 systems naming one of 38 initializers nothing declares — while vanilla and Ariphaos are **0**. A static map with **no** lanes is an error, not a valid shape — it used to be waved through on the grounds that 21 of STNH's 22 define none, and that is how a 95-system galaxy holding one hyperlane reported clean every time; those 21 build their network in a start-of-game script STG does not vendor ([94](../decisions/94-static-map-lanes-are-generated.md)). A scenario leaving `random_hyperlanes` on stays exempt. One condition is scoped to `stg_*` keys because *"the flag is the design key"* is our convention and vanilla answers it 21 times in 51 without a defect — [93](../decisions/93-static-galaxy-scenario.md) |
