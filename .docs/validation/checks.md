...
| `check_anomalies` | the four files an anomaly needs to agree on: the outcome event a category names, the sprite its picture names, and the six loc keys behind the log entry, the popup and the buttons | an anomaly is complete or it is a blank popup, and neither state dangles — [75](../decisions/75-trek-anomalies.md) |
| `check_archaeology` | the five files a dig site needs to agree on: the stage events and rune icons it names, its picture, the modifier its finale awards, the loc behind all of it — **and whether `stages = N` matches the `stage` blocks beside it** | a hand-written integer that nothing enforces and that makes the dig unfinishable when it is wrong; vanilla is 0 for 123 — [76](../decisions/76-trek-archaeology.md) |
