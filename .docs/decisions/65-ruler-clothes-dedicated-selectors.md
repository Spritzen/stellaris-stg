# 65 — A shared master selector cannot be indexed; give the ruler its own

**Status:** decided, 2026-08-08
**Supersedes** the earlier index model — *an absent `clothes` is index 0, and
index 0 is the file's first line* — which a live run falsified at all six
positions it set. **Closes** the loose end that model left open for TUR as well.
...
| Empire | class | selector | pinned | the index model predicted | what was drawn |
|---|---|---|---|---|---|
...
## What the index model got wrong, and the part that was right

It inferred the enumeration — *the distinct texture paths of the selector, in
...
empire's own species class — the rows [decision 20](20-empire-designer-clothes.md)
...
Commonwealth, which that pass reported and left unpinned because it is AI-only
...
## The second finding, which explains decision 54 as well
...
Decision 20 invented conditional rows in that scope, and
[decision 54](54-prescripted-rulers-unpin-clothes.md)'s observation — every
...
which is the failure [decision 31](31-duplicate-entity-declarations.md) names.
...
- **Whether the Federation president should wear the Starfleet formal robe at
  all.** The earlier pass raised this and it is unchanged: `federation_president_male_1`
...
Decisions 21, 54 and 68 are three attempts to write the right number into a
