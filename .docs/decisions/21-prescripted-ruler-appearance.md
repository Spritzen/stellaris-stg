# 21 — Prescripted rulers pin no appearance index

**Status:** decided, 2026-08-03
**Follows** [decision 20](20-empire-designer-clothes.md), which fixed the same
complaint in the empire designer and left the in-game starting ruler wrong.

## The report

After decision 20 shipped, the user reported that the *starting ruler* clothes
...
than the picker. Decision 20 had fixed the `game_setup` scope only.
...
one-species list. That is the same shape as decision 20's finding — *a shared
...
— which is the whole point of the trigger bodies decision 15 wrote and the
`game_setup` gating decision 20 added.
