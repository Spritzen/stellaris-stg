# 35 — Walshicus' station hulls carry no section attach points
...
## This is a different question from decision 28, and the names hide that

Both end in the word "locator" and they are not the same thing:

| | reads | asks about | failure |
|---|---|---|---|
| decision 28 / `check_asset_load_order` | `common/section_templates/` | gun mounts on a **section** entity | gun fires from the model origin |
| this one | `common/ship_sizes/` `section_slots` | attach points on a **hull** entity | the section never attaches at all |

Decision 28 deliberately excluded starbases and defence platforms from the
...
already uses, and spreading them would be the inventing decision 28 declined.
...
the declarations are honoured (decision 30).
