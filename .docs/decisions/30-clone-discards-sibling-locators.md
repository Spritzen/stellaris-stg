# 30 — `clone` discards every locator declared beside it

**Date:** 2026-08-07
**Status:** accepted
**Extends:** [28 — Weapon locators](28-weapon-locator-positions.md), whose repair
...
The 506 is the *same number decision 28 measured before its repair*. The repair
...
Decision 28 remains right that an `.asset` declaration satisfies the engine —
...
`clone` itself is not the problem and decision 17's load-order rule still binds:
...
- **Reading only the mesh binary to decide what a donor already carries was
  ...
  at `{ 0 0 0 }` as a mount when that is decision 28's silent defect. `has_mounts`
...
position its mesh bakes — decision 28's unresolved SHADOWED class. They are
