# 91 — STNH gets its Trek galaxy from static maps, not from the spawn lottery, and STG ships no map at all

**Status:** decided, 2026-08-26
**Follows** [decision 90](90-design-database-is-not-the-cause.md), which ruled out
the design database and left the lottery unexplained.
**Corrects** [decision 90](90-design-database-is-not-the-cause.md)'s reading of
...
**Corrects** [decision 52](52-trek-star-names.md)'s "ten hand-built galaxy maps":
STNH ships **22** `static_galaxy_scenario` files, 17 of which contribute a system
name. The 1,444 names decision 52 harvested are unaffected — the generator globs
the whole directory.
**Reframes** [decision 86](86-prescripted-empires-never-drawn.md) and this
project's whole approach to the question: `CUSTOM_EMPIRE_SPAWN_CHANCE` is a
...
## Why this also repairs decision 90's loose end

Decision 90 raised `randomized = no` on the species classes as the best remaining
...
the Federation's `spawn_enabled = always` never firing, which
[decision 88](88-playable-gates-the-design-database.md) closed and then had
falsified. It explains three empires, not 99. There is also a documented 4.3.3 /
...
system initializers** ([25](25-real-home-systems.md)) and the **1,444 Trek
systems placed by name in STNH's maps** ([52](52-trek-star-names.md)) that this
project has already harvested once for their names. The smallest STNH canon map
