# 67 — The artist's vocabulary is not the game's

**Status:** decided, 2026-08-08
**Fixes the tier-2 half of** [decision 60](60-mounts-share-existing-points.md),
...
them, so decision 60's tier 2 applies: round-robin the missing mounts over the
...
Re-derived across all 27 shipsets by decision 60's own procedure — delete the
...
been decision 28's defect returning, and it was not.
...
because one wanted mount is baked, and *the tiers do not mix* (decision 60), so
...
naming: every number decision 60 reported counts what the rule DID.** Mounts
...
That is decision 45's lesson in a fourth database: **the reverse question**. All
