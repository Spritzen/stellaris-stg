# 57 — A missing mount shares a point the artist drew

**Status:** decided, 2026-08-08
**Narrows** [decision 26](26-weapon-locator-positions.md), which gave every
...
small_gun_03   (2.480,  0.586, -1.805)   decision 26's bounding-box spread
```

Both of the artist's mounts are on the **centreline at the bow** — the forward
phaser strip, which is what a Trek ship's guns look like. Decision 26's third
...
anywhere, which is the same class of finding as decisions 40 and 46.
...
Re-derived across all 27 shipsets — the whole population, per decision 26's own
...
sections bake no locator at all, which is the defect decision 26 was written
...
- **`provenance()` read the wrong table.** It took the first row in
  ...
  is the two-column one it wants. Once [decision 33](33-station-section-attach-points.md)
