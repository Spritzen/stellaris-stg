# 76 — A quoted random name is a localisation key, and STG shipped 330 with no key

**Status:** decided, 2026-08-10
**Falsifies** the Trek star-name harvest in one part — its `token()` rule
stands, its "these are literals" reasoning does not.
**Follows** [decision 58](58-music-player-track-names.md), which is the same
...
[decision 58](58-music-player-track-names.md)'s finding — *a name that resolves
to itself still resolves* — in a third database, after the music player and the
shipset dropdown.

## Why the star-name harvest got it backwards
...
([decision 23](23-real-home-systems.md)) — a backlog from the tree
