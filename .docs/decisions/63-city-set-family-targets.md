# 63 — Read the resample target off vanilla's FAMILY, not only the file

**Status:** decided, 2026-08-08
**Closes the half** [decision 58](58-city-set-geometry.md) named and could not
...
Exactly the symptom decision 58 predicted would survive, on exactly the empire
...
([decision 49](49-flags-city-sets.md)), they were on the 560×280 canvas, and
...
is the property [decision 42](42-event-picture-geometry.md) refused to give up
...
- **`target: family` is opt-in per rule, not global.** Vanilla is 266/271
  ...
  past decision 42's calibrated scope. A widening should read as the piece of
...
reproduces decision 58's 153, which is what says the harness is measuring the
...
belongs to a family vanilla is uniform about. Decision 42's rule — derive, never
...
whole time and nobody asked it, which is decision 45's lesson arriving in a
