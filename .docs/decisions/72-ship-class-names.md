# 72 — STNH declares its own class names, so decision 59's fuzzy join was never needed

**Status:** decided, 2026-08-09
**Follows / closes the open half of** [decision 59](59-ship-name-pools.md).
**Uses** [decision 47](47-minor-power-names-truncated.md)'s rule on the way in.

## What was open

Decision 59 folded STNH's ship *registries* onto vanilla's ship sizes and left
...
tier, not a class. Decision 59 searched the localisation and never looked at the
name lists it was already parsing.

So the answer is a **direct harvest folded by decision 59's own tonnage table**,
...
key still stops the build, for decision 59's reason: a key with no rule is a
whole class list dropped in silence.

## Decision 59's exclusion is right for registries and wrong for classes
...
say.** The tonnage table is a judgement — decision 59 says so about the
...
- `UNDINE_CLASS_infestabami` resolves to **`infestabami`** — lowercase, and
  ...
  so decision 47's rule does not catch it, and the Undine are AI-only, so no log
