# 47 — 78 of 79 minor-power names shipped truncated, and 16 loc keys shipped as text
...
([decision 46](46-coalition-of-hope-takes-vul.md)), which needed the Coalition
...
from. This is decision 21's failure class — a silence failure in loc — one layer
further out.
...
- **A block-wide `^\s*name\s*=` regex is not the empire's name.**
  ...
  only. This is decision 33's lesson exactly: finding a name is not the same
...
- **Not every literal that matches a loc key is a loc key.** Resolving each
  ...
  decision 32's trap: there, refusing to read a quoted form deleted a real
...
**Calibrated in both directions**, per decision 30 — and this is the part
decision 43 got wrong by measuring something both hypotheses predicted:
...
decision 33's second paragraph. Re-breaking the file is the only thing that
distinguishes them.
...
That is decision 46's scope rule, applied to the file where someone will
