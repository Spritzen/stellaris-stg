...
| [111](111-three-galaxy-sizes.md) | The Known Galaxy comes in three sizes, and medium and large are vanilla's own star count and radius — *advances [static-galaxy-plan.md](../planning/static-galaxy-plan.md)'s step 6, the scale half. One generator, one `SIZES` row per size: **95 / 600 / 1,000** systems over a radius of **218 / 399 / 448**. Medium and large are vanilla's `medium` (600 at 400) and `huge` (1,000 at 450) matched on **both** numbers, which matches them on density too — **12.0 and 15.9** stars per 10,000 sq. units against vanilla's 11.9 and 15.9 — and density is what every distance in the game is tuned against. The scales are 0.66 and 0.74 because that is what lands the radius; `radius = 450` under vanilla's own *"less than 500, preferably less than ~460"* is why large is the top of the ladder. **`separation` is a threshold and its star count moves in steps** — 599 at 19.11, 603 at 19.10, no 600 in between — so each one overshoots by as little as it can and `trim` cuts **4 / 11 / 0** stars, one at a time, always the filler currently closest to another star and never an empire's seat. **Small is byte-identical** and keeps `default = yes` ([106](106-sealed-system-is-vanilla-content.md)) — and is now the odd one out, **a third the density of the other two**: it was measured against STNH's `09 botf`, and vanilla has no scenario that sparse. What survives that is the lane rule — mean degree **3.41 / 3.53 / 3.63**, longest lane **49 / 41 / 40** under vanilla's 50 — because `LANE_NEIGHBOURS` is a count and the cap only binds on the sparse map. The picker's loc file said `95 Star Systems` by hand; it is **generated now**. **No new check** — `check_static_galaxy` walks every file in the directory and re-proves connectivity over all 3,034 lanes. **Built and unrun***
...
| [106](106-sealed-system-is-vanilla-content.md) | The lane-less trinary star next to Ferenginar is vanilla's Sealed System, isolated on purpose, and the same save grades every question decision 87 left open — *a UFP run reported a trinary star near Ferenginar with no hyperlanes. It is `distar_sealed_1_2`, vanilla's **Sealed System**: `class = "rl_trinary_stars"`, spawned by `distant_stars_events_2.txt`, which calls **`isolate_system = yes`** on it and links it by a **wormhole pair** instead — bypass 2 `linked_to=3`, bypass 3 `linked_to=2`, `active=yes` on both. `error.log` held no record and never could: a lane-less system is not an engine error, the second time that shape has cost a session ([87](87-static-map-lanes-are-generated.md)). **Nothing to fix.** The same save answers all four of 87's closing questions: **99 of 100 systems reachable from Sol**, degree min 2 / max 5 / **mean 3.45** over the 95 scenario systems, and **zero `spawn_system` failures** against three on 2026-08-27 — all five runtime-spawned systems attached. Full log sweep: `make logs` **19 files, 0 warnings**; 1,735 records, **1,723 init / 12 post-init, none of the 12 ours**; the seven documented init classes reproduce **record for record, a third measurement**. But a fresh `.cpp:line` census returns **eight** — `game_singleobjectdatabase.h:170`, **172 records, the third-largest class, 172 in all three logs on disk, 0% ours**, missed because the table was reconciled against the classes already named rather than re-derived. And a class that looked like a new defect and is a **priced ack**: `pdx_entity.cpp:424`, 9 in every log, is [decision 35](35-attach-edges-into-pruned-art.md)'s acked naval-museum `attach` edges, unchanged at 9 records three weeks on — written up as a finding before `attach_target_ack` was read. **Find by re-deriving; classify by reading what is already recorded.***
| [107](107-the-ai-federation.md) | The Federation had no AI copy because Sol belongs to Real Space, and an `inline_script` reaches into a source's initializer without reproducing it — *closes [86](86-static-galaxy-scenario.md)'s first open content call, graded the same day by [106](106-sealed-system-is-vanilla-content.md). The map reserved system `0` for the Federation and **nothing created one**: the other 20 empires are made by their own initializer's guarded `create_country`, and Sol is **Real Space's file**, which no `src/` file can add a line to. All five existing questions passed and all five were right — they ask whether a join RESOLVES, and this one resolved to a file that creates nobody. 86 named two ways out; this is a third that costs neither: the block is **generated** into `common/inline_scripts/stg_federation_ai_empire.txt` by the same `ai_empire_block` the other 36 use, and one `patches:` line includes it, so Real Space keeps Sol's entity, flags and belts and the block cannot drift from the Federation's prescripted entry. **The anchor is 16 lines because the file holds SEVEN byte-identical Earths** — `flags = { planet_earth }` occurs 6 times, `sol_system_earth` 9, the entity name 7; uniqueness only arrives at Luna, and `count: 1` is what makes that safe. Sixth question added to `check_static_galaxy`, population 21, floor 0, **calibrated on three controls** — and the Klingon one, which broke an empire the fix had not touched, caught a substring test that called `stg_klingon_empire_TYPO` a match. Not graded: one glance at the contacts list asks whether there is an AI Federation, and exactly one* |
| [108](108-anomaly-weights-not-levels.md) | The Trek anomalies were written a notch stingier than vanilla in every body class, and the level skew that looked like the problem costs the player two points — *nothing had ever asked how often a player MEETS a Trek anomaly; `check_anomalies` asks only whether one is reachable ([74](74-reachability-checks.md)). The raw answer, **68 of 2,477 points of spawn weight — 2.7%**, is an artifact of four vanilla modifiers at `add >= 100`, `DISTAR_DEAD_GOD_CAT` alone at **1000**. Excluding them: **5.3% of the weight for 7.2% of the spawnable categories**, and **under fair share in every body class by the same factor** — medians vanilla 3.0, STG 2.0. Not a mistake in one place; a hand that wrote `add = 2` where vanilla writes 3. Remapped **1→2, 2→3, 3→5**, all 33 values, every target a vanilla idiom (vanilla writes 3 143 times), chosen from five candidates scored against fair share: lands **8.3% against 8.4%**, and within ~1.5 points per class. **The levels were left alone and that is the measured half.** The lean is real — 42.9% at level 5+ — but [73](73-phase-4-count-corrections.md)'s yardstick was vanilla's **base-game 40** (10% at 5+) where the pool a survey draws from is **327** (24.2%). Weight-weighted, the merged galaxy goes from **25.2% to 27.4%** at level 5+: two points, for a set that is 8% of the pool. A level is content — an Iconian gateway is meant to be hard. **No check**: a weight is a judgement and one would report a number forever, which is [104](104-script-documentation-is-a-version-exact-oracle.md)'s rule. Third instance of *the finding was right and the floor it was measured against was not** |
| [109](109-two-sources-one-loc-key.md) | The fourth quadrant of the contested-key hole was localisation between two sources, and the family filter is the whole check — *closes the item [92](92-src-contests-its-own-loc-keys.md) opened and left: `check_key_conflicts` walks `stg-build/common/` and asks its question of `key = { … }` blocks, and **a localisation file has no blocks**, so widening it was never a matter of adding a directory. Sibling check, the way [91](91-src-contests-its-own-name-lists.md) and 92 are siblings. Why it matters: `error.log` records a loc key that is MISSING and nothing at all about one two files declare — [45](45-minor-power-names-truncated.md) exactly, 78 of 79 minor powers reading `of Earth` with no log record. Raw, the merged tree contests **41 keys and 8 disagree** — and all 8 are Planetary Diversity overriding its own placeholders (`Placeholder Origin - DO NOT USE` → `Megaflora Tree of Life`), every one resolving to the extension under filename sort, which is [27](27-merge-semantics-per-directory.md)'s "extension wins". So `key_conflict_families` empties this check too, and the parser was **extracted to `_key_conflict_families()`** rather than copied. Population **27,742 keys**, floors vanilla 148,053-contesting-none and the tree 0 across families; LIOS always, no directory table. Same-value twins are **not** reported here though 92 reports them in `src/` — a duplicate of ours is a file to delete, a source's is not ours to tidy ([11](11-fix-source-errors-dont-drop.md)). **Four controls, and the first injection was wrong**: it picked the two sources with the most files, which were the same family, so the filter correctly ate it and the control read as "cannot fire" — **a control that does not know what the check filters is not a control*** |
| [110](110-effect-and-trigger-names-are-clean.md) | Our own script names no effect or trigger the engine does not have, and the merge-wide version of that question cannot be asked from the dump alone — *[104](104-script-documentation-is-a-version-exact-oracle.md)'s oracle, its other two files: **1,056 effects and 1,087 triggers**, asked of `src/` with a context-aware scanner that descends from effect and trigger entry points and resolves against the dump ∪ scopes ∪ the merge's **1,917 scripted effects and 2,143 scripted triggers** (vanilla **plus `dlc/*`**, or the floor is nonsense). **154 files, 14 unmatched tokens, every one a scanner artifact** — `NOT`/`NOR`, comparison operators, and effect SUB-FIELDS like the `flag`/`days` of `set_timed_country_flag`. Zero real findings across ~7,300 words of hand-written Trek script. **The build-wide version is blocked for a different reason**: the same scanner scores **48,622 unmatched tokens against vanilla itself**, topped by `id` 4,961 and `weight` 4,612 — the dump documents an effect's sub-fields only inside its free-text syntax example, so read as a flat allowlist the oracle models the language wrongly and reports itself, the identical failure 104 diagnosed. **Two halves failing for opposite reasons**: `src/` has nothing to catch, the merge has no oracle in the right shape. No check either way; the extraction regex, the DLC gotcha and the entry-point lists are recorded so nobody re-derives them* |
