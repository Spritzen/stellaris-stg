# 82 — The Republic of Hope and the Klingon-Cardassian Alliance are gone: two 2300s empires holding a second 40 Eridani and a second Qo'noS

**Status:** decided, 2026-08-25
**Closes** [decision 44](44-coalition-of-hope-takes-vul.md)'s subject — the
empire that took `VUL` no longer exists.
**Follows** the removal of the `playable = stg_never` gate, which put all 99 in
the AI pool and so made every duplicate home system a galaxy that holds the
same world twice.
...
separate copies of the same place, each with its own key. Under the
`playable` gate since removed, neither could ever load and it cost nothing.
Without the
...
4. **Species class `TNG`**, which is the part that would have been missed. Its
   own comment records it as claimed by no empire since decision 44 moved the
...
6. **18 empire localisation keys** and **54 species localisation keys** (27
   each for `TNG` and `TNGK`, the family [decision 19](19-species-class-localisation.md)
   requires).
...
[decision 47](47-flags-city-sets.md) put in front of the player, and a flag
