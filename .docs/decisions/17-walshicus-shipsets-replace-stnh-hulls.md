# 17 — Walshicus' shipsets replace STNH's hulls for nine of fourteen cultures

**Date:** 2026-08-03
**Status:** accepted
**Supersedes, in part:** [16 — STNH shipsets on a vanilla chassis](16-stnh-shipsets-on-a-vanilla-chassis.md)

## The problem decision 16 was solving, and why it could not be solved
...
vanilla's names. Decision 16 closed that gap by generating vanilla-shaped
...
the way decision 16 assumed.** All 268 entities reporting a missing locator
...
- **`interface/`** — every set ships a 3,391-line `interface/credits.txt` that
  shadows vanilla's 10,945-line one. Decision 07's failure mode exactly.
- **`common/species_classes/`** — each declares its own (`STARFLEET_TNG`,
  `KLINGON`, …). STG owns that namespace; the binding is one
  `graphical_culture =` line in `src/`, per [decision 09](09-species-class-keys-unprefixed.md).
...
[decision 22](22-group-c-texture-references.md)). `vendor.yml` is the list and
...
- **The prediction in the table above was exact.** The five kept cultures came in
  ...
  The run measured 506 because the minor-power harvest added four `generic_*`
  donor directories as cultures afterwards (+224). Nothing
  moved.
- **The locator check was rewritten and calibrated**: 0 false negatives against
  ...
  settled later by
  [decision 28](28-clone-discards-sibling-locators.md) — `clone` discards every
  ...
  and [26](26-weapon-locator-positions.md) for the repair.
- **A third, larger defect this decision introduced** was found by the same run:
  ...
  closed by [decision 22](22-group-c-texture-references.md), which found the
...
3. Everything in [decision 16's list](16-stnh-shipsets-on-a-vanilla-chassis.md)
