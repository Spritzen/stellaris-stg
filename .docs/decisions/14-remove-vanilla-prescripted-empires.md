# 14 — Remove vanilla's prescripted empires; pin `supported_version` exactly
...
[decision 12](12-fix-source-errors-dont-drop.md) accepts. It is not an error-count
...
[decision 08](08-stnh-art-shadows-vanilla.md) happened — and here it is exactly
...
- STNH's `default.txt` is a 3.12 copy that drops `trait = "trait_organic"` and
  ...
  decision 08's failure mode, in the one file §1 says to keep.
...
them in our build, with no error at load. That is decision 08, and it is
