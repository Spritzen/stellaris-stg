# 46 — Three findings from the 2026-08-07 fourth run, and the two checks they bought

**Status:** decided, 2026-08-07
**Extends** [decision 20](20-minor-power-species-class-keys.md) with a fourth
case, and [decision 33](33-duplicate-entity-declarations.md) with the texture
...
| 23:12:00 | `planet.cpp:2058` `PLANET_SCALE_SYSTEM` | known, acked, cosmetic — [decision 43](43-planet-scale-system-length.md). |
...
## 2. `TNG` → `VUL` — decision 20's fourth case, with a cost decision 20 did not have
...
`default` — human civilian clothes under a Vulcan face, decision 20's failure
exactly.

**Unlike decision 20's eight, `TNG` was not a misspelling.** It is a deliberate
...
starfleet_tng`. So this trade was real where decision 20's was free:
...
directions**, per decision 30: KRI and VALT stop reporting, TNG still reports,
and the benzite group — one of decision 20's original eight — still resolves to
its 6 real portraits on the master selectors, so respelling `BEN` back to `BENZ`
would still fire.

This is decision 33's lesson in a new place: *a scrape that finds names is not a
...
Nothing was checking it, and it is the texture analogue of decision 33: a
...
- **Content is compared, not names** (decision 29). It happens to eliminate
...
[decision 47](47-minor-power-names-truncated.md). 100 values, and the count in
