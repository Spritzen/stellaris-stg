# 51 — the other three prescripted-power files are clean; the truncation check stays where it is
...
That is the right instinct — "reads plausibly" is exactly what decision 47's
defect looked like — so the three files were swept before deciding anything.

## The sweep: 0 real findings

Both of decision 47's failure modes were run against all 22 empires in the three
...
Truncation is a *generator* failure — decision 47's names lost their leading
...
- **Truncation stays on `stg_z_minor_powers.txt` alone.** Widening it needs an
  ...
  47 left the six `Kessoks`/`Kessok` fields alone. Widening it would also mean a
...
check that reports a number and cannot fail — decision 33's trap, reached from
