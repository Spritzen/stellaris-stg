# 82 — Every Trek hull above corvette borrows a corvette's frame, and its sections hang on points a corvette does not have

**Status:** decided, 2026-08-10
**Follows** [decision 35](35-station-section-attach-points.md), which is the
same defect one entity family over and whose check was scoped to stop exactly
where this begins.
**Follows** [decision 60](60-mounts-share-existing-points.md) — a mount is no
...
**Word for word decision 35's message**, which named two Walshicus *station*
...
- **Never a file `vendor.yml` patches.** An `src/` override replaces the
  ...
  outright. 28 files are reported and left, all of them decision 35's own
  station patches.
...
`check_section_attach_points` asks precisely this question, and decision 35
