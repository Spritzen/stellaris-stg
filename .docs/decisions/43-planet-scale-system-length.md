# 43 — `PLANET_SCALE_SYSTEM` is measured against an array no script can set
...
[Decision 11](11-drop-cinematic-camera-and-ambient-soundtracks.md) dropped
...
- **`len == len(ZOOM_STEPS_SYSTEM_PERCENTAGES)`** — what decision 11 and
...
step that has never existed — but the message decision 11 pinned on it was
...
- The engine cannot map zoom step → planet scale, and falls back to a default.
  ...
  (decision 11), so under this reading we have been shipping the mod with its
  main feature inert.
...
The lesson is decision 33's, in a new place: *a measurement that both hypotheses
...
it was gone, and [decision 11](11-drop-cinematic-camera-and-ambient-soundtracks.md)
...
What restoring it *does* do is the argument decision 11 made second and buried:
...
scale ramp. That is re-tuning someone else's camera design, and decision 11's
...
same defect. Decision 11 even records the calibration: *"Verified by restoring
