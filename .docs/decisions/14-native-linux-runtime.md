# 14 — Compatibility mode off: the game is the native Linux build, and the user-data folder moved back
...
[decision 06](06-launcher-local-mod-registration.md). It does not reverse its
...
`stellaris.exe` and no `.dll` left — the inverse of the table in decision 06.
...
That is the folder decision 06 called "dead since 2025-11-09". It is dead no
...
- **`launcher_path()`** renders `path=` per platform — the plain host path for
  native Linux, decision 06's `C:` drive-letter translation for Proton.
- **`check_mount_matches_platform()`** refuses to deploy when the mounted
  ...
  again; without the guard that drift silently reproduces decision 06's exact
...
The symlink half of [decision 12](12-build-dir-and-symlink-deploy.md) is
...
- **The registry entry survived.** `launcher-v2.sqlite` here holds 38 mods
  ...
  decision 06 diagnosed as going to the "dead" folder — that deploy *did*
...
- **`requiredVersion` is the stale `4.4.*`**, from the Aug-1 descriptor, not the
  `v4.4.6` that decision 06's addendum established. The deployed `.mod` now
...
Decision 06's rule 1 — *an artefact's timestamp is part of its evidence* — is
