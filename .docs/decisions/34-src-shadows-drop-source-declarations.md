# 34 — An `src/` override shadows the *sources* too, not just vanilla
...
second, because `src/gfx/FX/pdxmesh.shader` — written for decision 08 as
...
(decision 33); the `Effect`/state declaration line is identity in a `.shader`.
