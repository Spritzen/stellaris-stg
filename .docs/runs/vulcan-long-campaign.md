...
[decision 76](../decisions/76-random-names-are-loc-keys.md) and
[decision 77](../decisions/77-hull-section-attach-points.md). **Nothing either of
...
- [done] **Turn ironman OFF, or `autosave_tocloud` off, for this run.** Done for
      ...
      ([decision 83](../decisions/83-design-database-is-not-the-cause.md)). `settings.txt` no
...
- [ ] **Read the AI empire preview before you press start.** The galaxy setup
      ...
      **Still outstanding, and now the only cheap thing left.** 18 *Random AI
      ...
      ([83](../decisions/83-design-database-is-not-the-cause.md)) — so the glance
...
- **T'Pau's clothes.** Her row names `portrait = stg_vul_ruler`,
      ...
      [decision 65](../decisions/65-ruler-clothes-dedicated-selectors.md)**: if
...
- **Then walk the clothes slider a long way** — 1, 20, 100, 300, 480, 499 —
      ...
      ([80](../decisions/80-selector-textures-that-resolve.md)) — so a wrap seen
...
- **The room behind her** is `vulcan_room` — a Trek room, not a vanilla
      personality room ([46](../decisions/46-room-selector-merge.md)).
```markdown
#OBSERVATIONS
...
- **Walk the home system once and read every body's name.** It should be
      **40 Eridani**, capital **Vulcan** (a desert world), with T'Khut,
      ...
      ([23](../decisions/23-real-home-systems.md)). **A name showing as a raw key
...
- **One pass over the galaxy map, reading names.** 328 localisation keys
      ...
      **Class 9 Nebula** and **Kullat Nunu**. **Any remaining underscore is a
      ...
      ([76](../decisions/76-random-names-are-loc-keys.md)).
```markdown
#OBSERVATIONS
...
the container can grade any of it** ([70](../decisions/70-trek-anomalies.md)).
...
- [ ] **Does the picture match the text, and is it framed right?** This is the
      ...
      behind it** ([69](../decisions/69-event-picture-families.md)). **A subject
...
- [ ] **Do they turn up at all?** Sites are placed by vanilla's `ancrel.9999` on
      ...
      not the content ([71](../decisions/71-trek-archaeology.md)).
...
[decision 74](../decisions/74-reachability-checks.md) was written against.
...
([77](../decisions/77-hull-section-attach-points.md)). Before that fix, every
...
- [ ] **Then the pattern question, which is the harder one.** 66% of mounts
      ...
      ([64](../decisions/64-source-art-hardpoint-names.md)); the thing to look
...
Read [decision 67](../decisions/67-ship-class-names.md) and
[68](../decisions/68-class-name-thematic-fill.md) before grading this, because
...
- [ ] **Read the Vulcan class names and say whether they read as Vulcan.**
      ...
      [decision 68](../decisions/68-class-name-thematic-fill.md) chose. **Say
...
- [ ] **Then the registries, which are a separate pool.** The Vulcan corvette
      ...
      ([56](../decisions/56-ship-name-pools.md),
...
- [ ] **The capital's city view, once it has grown.** `vulcan_01` is the tallest
      city set in the tree and the one whose framing has been reviewed twice
      ([63](../decisions/63-city-set-canvas-overflow.md),
      [81](../decisions/81-city-horizon-band.md)). **The right-hand
...
- [ ] **Build a habitat and look at it.** Six `vendor.yml` renames changed which
      ...
      **whether habitats draw as vanilla's orbital ring rather than a Suliban
      helix has never been seen** ([50](../decisions/50-duplicate-entity-triage.md)).
...
- [ ] **The count is 55 names in the player and 27 in the rotation.** Not 22, and
      ...
      respectively, and both are correct ([58](../decisions/58-music-player-track-names.md),
      [62](../decisions/62-music-rotation-dedupe.md)). **Do not re-report a count
...
- [ ] **The Federation anthem is in the ambient rotation for a Vulcan empire
      ...
      ([52](../decisions/52-federation-anthem.md)). Confirming that is a
