...
SKIP = {
    ...
    # Its AI copy is emitted separately -- see INLINE_AI.
...
# Empires whose home system belongs to a VENDORED SOURCE rather than to us, so
# there is nowhere in src/ to put the guarded create_country that makes their AI
# copy. The block is written to its own file here and spliced into that source's
# initializer by a `patches:` entry in vendor.yml, using the engine's own
# `inline_script` include -- vanilla's mechanism, used in
# common/solar_system_initializers/ by distant_stars_initializers.txt and eight
# other files.
#
# THE FEDERATION IS THE ONLY ONE, and it is the reason this exists. Sol is Real
# Space's `sol_system_initializer`, not ours; decision 86 named two ways to
# reach it -- a patch carrying the whole block, or an STG-owned copy of Sol with
# the Federation repointed at it -- and this is a third that costs neither. Real
# Space keeps Sol (decision 23's "pointing at it beats reproducing it" survives
# intact, and Earth keeps Real Space's own entity, flags and belts), while the
# block itself stays GENERATED from the Federation's prescripted entry, so it
# cannot drift from the empire it copies the way a block hand-written into
# vendor.yml would. The patch inserts one line.
#
# See .docs/decisions/107-the-ai-federation.md.
INLINE_AI = {
    "stg_united_federation_of_planets":
        REPO / "src/common/inline_scripts/stg_federation_ai_empire.txt",
}
...
def main() -> int:
    ...
    inlined = []
    for stg_key, out_path in sorted(INLINE_AI.items()):
        emp = empires.get(stg_key)
        if emp is None:
            print(f"ERROR: INLINE_AI names {stg_key}, which no prescripted "
                  f"empire declares. Nothing written.", file=sys.stderr)
            return 1
        frag = f"""\
# GENERATED by tools/gen_home_systems.py -- do not hand-edit; regenerate.
#
# The AI copy of {stg_key}, as an inline_script fragment.
#
# This empire's home system belongs to a vendored source rather than to us, so
# the guarded create_country below has nowhere in src/ to live. vendor.yml
# splices this file into that initializer's capital with one
# `inline_script = "{out_path.stem}"` line, and the engine includes it
# verbatim -- the same include vanilla uses nine times in
# common/solar_system_initializers/.
#
# The block is the SAME ai_empire_block the other 36 home systems carry, read
# from this empire's own prescripted_countries/ entry, so it cannot drift from
# the empire it copies. Everything the guard does and why is documented on
# ai_empire_block in the generator.
#
# See .docs/decisions/107-the-ai-federation.md.
...
"""
        ...
        inlined.append((stg_key, out_path))
...
# which Real Space rescales, and which is already the real solar system. Its AI
# copy IS in the tree — as common/inline_scripts/stg_federation_ai_empire.txt,
# generated by the same ai_empire_block as the 36 below and spliced into Real
# Space's Earth by a vendor.yml patch, because a source's initializer is the one
# place src/ cannot reach (.docs/decisions/107-the-ai-federation.md).
#
...
'''
    ...
    for stg_key, out_path in inlined:
        print(f"wrote {out_path.relative_to(REPO)}  ({stg_key} AI copy, "
              f"spliced by a vendor.yml patch)")
