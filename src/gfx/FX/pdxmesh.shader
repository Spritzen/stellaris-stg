# Star Trek Galaxies -- OVERRIDE of vanilla gfx/FX/pdxmesh.shader.
#
# Keeps the vanilla filename deliberately: this file must shadow vanilla's to be
# read at all. Required header per .docs/architecture/vendored-merge.md rule 4.
#
# WHAT THIS OVERRIDES AND WHY
# ---------------------------
# This is vanilla 4.4's pdxmesh.shader, byte-for-byte, with TWO marked blocks
# appended at the end -- five effects from STNH, four from Real Space. Nothing
# vanilla defines is removed or changed.
#
...
# PdxMeshShipHue120Shadow, PdxMeshWaterShipShadow. Without them the 2026-08-01
# live run threw 399 `Failed to create material with shader ...` errors, and the
# affected materials are hull stripes, nacelle stripes, faction emblems and
# saucer decals -- visible Trek identity on the shipsets Phase 3 wired.
#
# WHY NOT JUST VENDOR STNH's COPY
# -------------------------------
...
# diff rather than a silent revert.
#
# When Stellaris patches this file, re-derive it:
#   diff <(tr -d '\r' < /stellaris/gfx/FX/pdxmesh.shader) \
#        <(sed '1,/END STG HEADER/d' src/gfx/FX/pdxmesh.shader)
#
# NOTE ON HUE_SHIFT_120: vanilla 4.4's shader body defines HUE_SHIFT,
# RIM_LIGHT, PDX_IMPROVED_BLINN_PHONG and NO_ALPHA_MULTIPLIED_EMISSIVE, but not
# HUE_SHIFT_120 -- and neither does STNH's own body. It is an inert define in
# both trees, carried verbatim so the block matches upstream. PdxMeshShipHue120
# therefore renders as a plain hue-shifted ship shader, which is what it does
# under STNH too.
#
...
# -----------------------------------------------------------------------------
# BEGIN STG ADDITIONS -- the five effects STNH ship meshes require.
# Copied verbatim from .source/688086068/gfx/FX/pdxmesh.shader. Everything above
# this line is vanilla 4.4. Nothing vanilla declares has been altered.
# -----------------------------------------------------------------------------

###Shifted 120 degrees###
Effect PdxMeshShipHue120
{
	VertexShader = "VertexPdxMeshStandard"
	PixelShader = "PixelPdxMeshShip"
	Defines = {
...
Effect PdxMeshShipHue120Shadow
{
	VertexShader = "VertexPdxMeshStandardShadow"
	PixelShader = "PixelPdxMeshStandardShadow"
	Defines = {
